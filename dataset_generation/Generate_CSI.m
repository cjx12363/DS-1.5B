...
UEAntArray = qd_arrayant.generate('3gpp-mmw',1,4,...  % UE=1x4 ULA
    s.center_frequency,1,ElcTltAgl_BS,...
    Vspc_Tx_BS/s.wavelength,Mg_BS,Ng_BS,...
    Vspc_Tx_BS/s.wavelength*M_BS,Hspc_Tx_BS/s.wavelength*N_BS);

Speed=10.1:0.1:100; %[km/h]
H_U_his = zeros(900,10,16,48,4,4,4,2);  % +Nr=4 dim
H_U_pre = zeros(900,10,4,48,4,4,4,2);  % +Nr=4 dim
H_D_pre = zeros(900,10,4,48,4,4,4,2);  % +Nr=4 dim
...
%     l1.set_scenario('Freespace');
    ...
    for ii=1:UENum
        ...
        h=reshape(h,2,4,4,4,96,20);  % 2pols x 4x4 BS x 4 UE x 96SC x 20T
        h=permute(h,[6,5,4,3,2,1]);  % T K BS_M BS_N Nr pol
...
save('E:\\cjx12363\\LLM4CP-DS\\H_U_his_train.mat', 'H_U_his', '-v7.3');
save('E:\\cjx12363\\LLM4CP-DS\\H_U_pre_train.mat', 'H_U_pre', '-v7.3');
save('E:\\cjx12363\\LLM4CP-DS\\H_D_pre_train.mat', 'H_D_pre', '-v7.3');
fprintf('Saved to E:\\cjx12363\\LLM4CP-DS\\\n');
