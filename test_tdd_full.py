...
pre_raw = load_mat(f'./data/test/{args.scenario}_H_U_pre_test.mat', 'H_U_pre')
...
model.load_state_dict(torch.load('./Weights/LLM4CP_DS.pth', map_location=device)); model.eval()
print(f'TDD {args.scenario} SNR={args.snr_dl}dB | MRT+MRC')
...
with open(time.strftime('result/%Y%m%d_%H%M%S')+'_nmse_tdd.csv', 'w') as f:
